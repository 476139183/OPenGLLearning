...
@property (nonatomic, strong) NSMutableArray <SceneCar *>*cars;
...
- (void)setUp {
  ...
  /* 之所以需要设置一个目标位置，是为了在视角切换
   通过高通滤波器函数SceneVector3FastLowPassFilter和低通滤波器函数SceneVector3SlowLowPassFilter，实现视角平滑过渡。
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
  ...
  self.baseFffect.transform.modelviewMatrix = GLKMatrix4MakeLookAt(
                                                                   self.eyePosition.x,
                                                                   self.eyePosition.y,
                                                                   self.eyePosition.z,
